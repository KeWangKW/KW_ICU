...
class KWNavigationController: UINavigationController {
    ...
    //状态栏文字颜色
    //需在info.plist 中增加 View controller-based status bar appearance , 并且设置为YES
...
public extension UINavigationController {
    ///修改状态栏颜色
    func KWChangeStatusBarStyle(_ style:UIStatusBarStyle) {
        if let nav = self as? KWNavigationController {
            nav.StatusBarStyle = style
